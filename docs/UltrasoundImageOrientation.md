...
![Ultrasound image orientation transducer axes](images/UltrasoundImageOrientationTransducerAxes.png)
...
(based on the `PortUsImageOrientation` attribute of the `Device` element in the device set configuration file).
...
## Example
The image `x` axis points towards the same direction as the `U` (unmarked) transducer axis. The image `y` axis points towards the same direction as the `F` (far) transducer axis. Therefore, the ultrasound image orientation is: `UF.`

![Ultrasound image orientation example](images/UltrasoundImageOrientationExample.png)
...
![Ultrasound image orientations segmentation parameter window](images/UltrasoundImageOrientationsSegParamWindow.png) 
...
![Ultrasound image orientation Ultrasonix software](images/UltrasoundImageOrientationUltrasonixSw.png)