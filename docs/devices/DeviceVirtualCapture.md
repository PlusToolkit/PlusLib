...
## Device configuration settings
  ...
  - **DeviceType**: (`Type`) = `(`VirtualCapture`)` (Required)
  - **DeviceAcquisitionRate**: (`AcquisitionRate`) defines how frequently the device copies frames from the input data source to the disk. (Optional, default: 30)
  - **LocalTimeOffsetSec**: (Optional, default: 0)


  - **BaseFilename**: File to write, path relative to output directory.
  - **EnableFileCompression**: Flag to write it compressed. ( - Warning! Beware file limits on old FAT32 disks (4GB maximum file size)
