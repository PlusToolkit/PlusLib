...
### OEM interface
  - **DeviceType**: Must be `BkProFocusOem`. (Required)

  - **DeviceAcquisitionRate**: (`AcquisitionRate`). Image retrieval may slow down the exam software, so keep the frame rate low by default (Optional, default: 1)
