...
## Device configuration settings
  ...
  - **DataSources**: Exactly one `DataSource` child element is required. (Required)
     - **DataSource**: (Required)
      ...
      - **`ImageType`. Color image acquisition is supported by setting the imaging device's common attribute `ImageType=(`RGB_COLOR`)` as shown in the Epiphan Color Video Capture example. (Optional, default: BRIGHTNESS)
