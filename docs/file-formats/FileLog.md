...
The directory where log files are saved is specified in the OutputDirectory attribute in the `FileApplicationConfiguration`.
If a relative path is specified in the OutputDirectory attribute then it is interpreted as relative to the location of
the `FileApplicationConfiguration` .

## Viewing log files

Any text editor can be used for viewing the log files, but a log viewer software is recommended.

The log file format is compatible with the free [LogExpert log viewer](http://logexpert.codeplex.com/).
...
![LogExpert screenshot](../images/FileLogLogExpertScreenshot.png) 
...
![LogExpert configuration 1](../images/FileLogLogExpertConfig1.png)

![LogExpert configuration 2](../images/FileLogLogExpertConfig2.png)