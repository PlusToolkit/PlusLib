...
void* vtkPlusOpenIGTLinkClient::DataReceiverThread( vtkMultiThreader::ThreadInfo* data )
{
  ...
  while ( self->DataReceiverActive.first )
  {
    ...
    //check if the message type is TRANSFORM
    else if (strcmp(headerMsg->GetDeviceType(), "TRANSFORM") == 0)
    {
	  
      igtl::TransformMessage::Pointer replyMsg = igtl::TransformMessage::New(); 
      replyMsg->SetMessageHeader(headerMsg); 
      replyMsg->AllocatePack(); 
      {
        PlusLockGuard<vtkRecursiveCriticalSection> socketGuard(self->SocketMutex);
      ...
      }
      {
        ...
        igtl::Matrix4x4 mx;
        replyMsg->GetMatrix(mx);
        //LOG_INFO("matrix for "<<replyMsg->GetDeviceName()<<" TRANSFORM received : ");
        //igtl::PrintMatrix(mx);
      }      
    }
    else
