...
int main( int argc, char** argv )
{
 double runTime = 0;
...
#ifdef _WIN32
  std::cout << "Press C or c or Ctrl-C to quit:" << std::endl;
#else
  ...
  neverStopClient = (runTime==0.0);

  // Run client until requested 
  const double commandQueuePollIntervalSec=0.010;
  while ( (vtkAccurateTimer::GetSystemTime() < startTime + runTime) || (neverStopClient) )
  {
    if( GetAsyncKeyState('C') || GetAsyncKeyState('c') )
    {
      LOG_ERROR("Disconnecting client...");
      client->Disconnect();
      LOG_ERROR("Client disconnected.");
      runTime = 0.0;
      neverStopClient = false;
    }
