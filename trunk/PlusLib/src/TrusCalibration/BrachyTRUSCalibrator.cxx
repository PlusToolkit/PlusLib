...
PlusStatus BrachyTRUSCalibrator::addDataPositionsPerImage( 
	...
	const vnl_matrix<double> TransformMatrixUSProbe2Stepper4x4 )
{
  ...
  if( mIsPhantomGeometryLoaded != true )
	{
		LOG_ERROR("The phantom geometry has not been loaded!");
    return PLUS_FAIL;
	}

	if( SegmentedDataPositionListPerImage.size() != mNUMREFPOINTSPERIMAGE )
	{
		LOG_ERROR("The number of N-wires is NOT "	<< mNUMREFPOINTSPERIMAGE << " in one US image as required!");
    return PLUS_FAIL;
	}
...
PlusStatus BrachyTRUSCalibrator::addValidationPositionsPerImage( 
	std::vector<vnl_vector_double> SegmentedDataPositionListPerImage, 
	const vnl_matrix<double> TransformMatrixUSProbe2Stepper4x4 )
{
  static int frameIndex=-1;
