...
int main(int argc, char **argv)
{
	...
	defaultParams.CalibrationFileName = "template_calibration.xml";
	...
	defaultParams.ImageSendPeriod = 500.0;
	defaultParams.SendTransforms = false;
	defaultParams.TransformSendRate = 20.0;
#ifdef _WIN32
	defaultParams.SaveDirectory = "..\\..\\..\\USTemplateCalibrationOutput";
#else
	defaultParams.SaveDirectory = "../../../USTemplateCalibrationOutput";
#endif
	...
	vtkCalibDataProcessor *processor = vtkCalibDataProcessor::New();
	vtkCalibDataSender *sender = vtkCalibDataSender::New();
	...
	else
	{
		...
		vtkUltrasoundDataCollector *usDataCollector = vtkUltrasoundDataCollector::New();
		...
		{
			usDataCollector->SetSaveInsertedDirectory(reconParams.SaveDirectory);
			usDataCollector->SaveInsertedSlicesOn();
			usDataCollector->SaveInsertedTimestampsOn();
		}
		else if (reconParams.SaveTimestamps)
		{
			usDataCollector->SetSaveInsertedDirectory(reconParams.SaveDirectory);
			usDataCollector->SaveInsertedTimestampsOn();
			usDataCollector->SaveInsertedSlicesOff();
		}
		else
		{
			usDataCollector->SaveInsertedSlicesOff();
			usDataCollector->SaveInsertedTimestampsOff();
		}
		...
		usDataCollector->SetVideoSource(videoSource);
		usDataCollector->SetSlice(videoSource->GetOutput());
		usDataCollector->SetTrackerTool(tracker->GetTool(reconParams.TrackerPort));

		if (!usDataCollector->ReadSummaryFile(reconParams.CalibrationFileName))
		{
		...
		processor->SetUSDataCollector(usDataCollector);
		...
		//TODO: Sender should send data to the UI
		...
		sender->ConnectToImageViewer(); 
		iren->SetRenderWindow(sender->GetImageViewer()->GetRenderWindow());
		sender->GetImageViewer()->SetupInteractor(iren);
		// start send thread
		if(terminate == 0)
		{
		...
		if (terminate == 0)
		{
			...
			usDataCollector->StartRealTimeReconstruction();
		}

		//if (terminate == 0)
		//{
		//	// TODO: Synchronization
		//	usDataCollector->SyncronizeUSToTracker(); 
		//}
		...
		//  usDataCollector->StopRealTimeReconstruction();
		//  }
		...
		//  usDataCollector->FillHolesInOutput();
		...
		//  int numSent = 0;
		//  int prevNumSent = 0;
		//  int numNeeded = usDataCollector->GetNumberOfOutputVolumes();
		...
		//  {
		//  cout << "Closing image viewer connection " << endl << endl;
		//  sender->CloseViewerConnection();
		//  }
		...
		if (terminate == 0)
		{
			...
			cout << "Saving data files to " << reconParams.SaveDirectory << endl;
			usDataCollector->SaveDataToFolder(reconParams.SaveDirectory);
			usDataCollector->SaveSummaryFile(reconParams.SaveDirectory);

			// save output volumes
			//cout << "Saving " << usDataCollector->GetNumberOfOutputVolumes() << " output volumes():" << endl;
			//vtkImageExtractComponents *extract = vtkImageExtractComponents::New();
			//vtkDataSetWriter *writer3D = vtkDataSetWriter::New();
			//char filename[512];
			//int phase;
			//for (int port = 0; port < usDataCollector->GetNumberOfOutputVolumes(); port++)
			//{
			// set filename
			//      phase = usDataCollector->CalculatePhaseFromPort(port);
			//#ifdef _WIN32
			...
			//      extract->SetInput(usDataCollector->GetOutputFromPort(port));
