...
vtkCxxRevisionMacro(vtkCalibDataSender, "$Revision$");
vtkStandardNewMacro(vtkCalibDataSender);

//----------------------------------------------------------------------------
vtkCalibDataSender::vtkCalibDataSender()
{
  this->SendPeriod = 1.0;
...
//----------------------------------------------------------------------------
void vtkCalibDataSender::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
int vtkCalibDataSender::ConnectToImageViewer()
{
	...
	this->TextActor->GetTextProperty()->SetColor(1,0,0);
	this->TextActor->GetTextProperty()->SetFontFamilyToArial();
	this->TextActor->GetTextProperty()->SetFontSize(15);
	...
	this->TextActor->SetDisplayPosition(10,50); 
	
	this->ImageViewer->GetRenderer()->AddActor(this->TextActor); 
...
//----------------------------------------------------------------------------
int vtkCalibDataSender::CloseViewerConnection()
{
...
static void *vtkDataSenderThread(vtkMultiThreader::ThreadInfo *data)
{
  vtkCalibDataSender *self = (vtkCalibDataSender *)(data->UserData);

  double sendPeriod = self->GetSendPeriod();
  int currentIndex = -1;
  SenderDataStruct imageMessage;
  int errors = 0;
...
//----------------------------------------------------------------------------
bool vtkCalibDataSender::IsSendDataBufferEmpty()
{
  return this->SendDataQueue.empty();
}

//----------------------------------------------------------------------------
bool vtkCalibDataSender::IsSendDataBufferFull()
{
...
//----------------------------------------------------------------------------
int vtkCalibDataSender::GetHeadOfNewDataBuffer()
{
...
//----------------------------------------------------------------------------
int vtkCalibDataSender::IncrementBufferIndex(int increment)
{
...
//----------------------------------------------------------------------------
int vtkCalibDataSender::StartSending()
{
   if (!this->Connected)
    {
    if(!this->ConnectToImageViewer())
      {
      return 0;
      }
    }
...
//------------------------------------------------------------
int vtkCalibDataSender::StopSending()
{
  //Stop send thread
  if (this->Sending)
    {
...
//------------------------------------------------------------
int vtkCalibDataSender::NewData(vtkImageData* frame, vtkMatrix4x4* trackerMatrix, int phase)
{
...
//------------------------------------------------------------
int vtkCalibDataSender::PrepareImageMessage(int index,
                                       SenderDataStruct& imageMessage)
{
  ...
  //Get Data
  imageMessage = this->SendDatabuffer[index];
  //imageMessage.ImageData = this->SendDatabuffer[index].ImageData;
  //imageMessage.Matrix = this->SendDatabuffer[index].Matrix;
  //imageMessage.Phase = this->SendDatabuffer[index].Phase;
...
//------------------------------------------------------------
int vtkCalibDataSender::SendMessage(SenderDataStruct& message)
  {

	  //Send message over OpenIGTLink connection
	  //double sendTime = this->GetUpTime();
	  //this->ImageViewer->SetInput(message.ImageData); 

	  //std::ostringstream ss;
	  //message.Matrix->Print(ss); 
...
//	  char path[512];
//#ifdef _WIN32
//	  sprintf(path, "D:\\CALIBDATA\\z%d.bmp", this->SaveInsertedTimestampsCounter++);
//#else
//	  sprintf(path, "%s/z%d.bmp", this->SaveInsertedDirectory, "frameTimestamps.txt");
//#endif
//	  std::cout<<this->SaveInsertedTimestampsCounter<<std::endl; 
//
//	  vtkBMPWriter* SaveInsertedSlicesWriter = vtkBMPWriter::New(); 
//	  SaveInsertedSlicesWriter->SetFileName(path);
//	  SaveInsertedSlicesWriter->SetInput(message.ImageData);
//	  SaveInsertedSlicesWriter->Update();
//	  SaveInsertedSlicesWriter->Write();
//	  SaveInsertedSlicesWriter->Delete(); 
...
//------------------------------------------------------------
void vtkCalibDataSender::DeleteAllData()
  {
  int index;
...
//------------------------------------------------------------
int vtkCalibDataSender::TryToDeleteData(int index)
{
  if(this->IsIndexAvailable(index) || index < 0 || index >= this->SendDataBufferSize)
    {
    #ifdef  ERRORSENDER
...
//------------------------------------------------------------
int vtkCalibDataSender::UnlockData(int index, int lock)
{
  if(this->IsIndexAvailable(index) || index < 0 || index >= this->SendDataBufferSize)
    {
    #ifdef  ERRORSENDER
...
/******************************************************************************
 * int vtkCalibDataSender::LockIndex(int index, int requester)
 *
 *  Lock index "index"
 *
 *  @Author:Jan Gumprecht
...
/******************************************************************************
 * int vtkCalibDataSender::ReleaseLock(int requester)
 *
 *  Release Lock
 *
 *  @Author:Jan Gumprecht
...
//------------------------------------------------------------
int vtkCalibDataSender::AddDatatoBuffer(int index, vtkImageData* imageData, vtkMatrix4x4* matrix, int phase)
{
...
/******************************************************************************
 * double vtkCalibDataSender::GetUpTime()
 *
...
/******************************************************************************
 * bool vtkCalibDataSender::IsIndexAvailable(int index)
 *
...
/******************************************************************************
 * void vtkCalibDataSender::SetLogStream(ofstream &LogStream)
 *
 *  Redirects Logstream
 *
 *  @Author:Jan Gumprecht
...
/******************************************************************************
 * ofstream& vtkCalibDataSender::GetLogStream()
 *
 *  Returns logstream
 *
 *  @Author:Jan Gumprecht
 ...
 *
 * ****************************************************************************/
void vtkCalibDataSender::UpdateFrameRate(double sendTime)
{
  this->UpdateCounter++;
