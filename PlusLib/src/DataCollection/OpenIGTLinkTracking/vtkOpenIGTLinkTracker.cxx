...
PlusStatus vtkOpenIGTLinkTracker::InternalUpdate()
{
  ...
  else if (strcmp( headerMsg->GetDeviceType(), "TDATA" ) == 0 )
  {
    ...
    for ( int i = 0; i < tdataMsg->GetNumberOfTrackingDataElements(); ++ i )
    {
      ...
      if ( this->ToolTimeStampedUpdateWithoutFiltering(transformName.From().c_str(), toolMatrix, TOOL_OK, unfilteredTimestamp, filteredTimestamp) !=
        PLUS_SUCCESS )
      {
        LOG_ERROR("ToolTimeStampedUpdate failed for tool: " << transformName.From() << " with timestamp: " << std::fixed
          << unfilteredTimestamp); 
