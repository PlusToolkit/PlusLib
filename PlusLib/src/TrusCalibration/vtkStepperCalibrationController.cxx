...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::LSQRMinimizer(std::vector<vnl_vector<double>> aMatrix, std::vector<double> bVector, vnl_vector<double> &resultVector)
{
...
int vtkStepperCalibrationController::CalibrateProbeRotationAxis()
{
	...
	{
		LOG_WARNING("Unable to calibrate probe rotation axis!"); 
	}
	...
	this->RotationEncoderCalibration(); 
	...
	this->CalculatePhantomToProbeDistance(); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::CalibrateProbeTranslationAxis()
{
	...
	this->TranslationAxisCalibration(PROBE_TRANSLATION); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::CalibrateTemplateTranslationAxis()
{
	...
	this->TranslationAxisCalibration(TEMPLATE_TRANSLATION); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::TranslationAxisCalibration( IMAGE_DATA_TYPE dataType )
{
	...
	do 
	{
		...
		this->LSQRMinimizer(aMatrix, bVector, translationAxisCalibResult); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::GetTranslationAxisCalibrationError(const std::vector<vnl_vector<double>> aMatrix, const std::vector<double> bVector, const vnl_vector<double> resultVector, std::vector<double> &mean, std::vector<double> &stdev )
{
	...
	for ( int i = 0; i < posDifferences.size(); i++ )
	{
	...
	// calculate standard deviation for each wire 
	stdev.clear(); 
	for ( int i = 0; i < diffFromMean.size(); i++ )
	{
	...
	// print results
	for ( int i = 0; i < numOfSegmentedPoints; i++ )
	{
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::SaveTranslationAxisCalibrationError(const std::vector<vnl_vector<double>> aMatrix, const std::vector<double> bVector, const vnl_vector<double> resultVector, IMAGE_DATA_TYPE dataType)
{
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::RemoveOutliersFromTransAxisCalibData(std::vector<vnl_vector<double>> &aMatrix, std::vector<double> &bVector, vnl_vector<double> resultVector )
{
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::GetRotationEncoderCalibrationError(const std::vector<vnl_vector<double>> aMatrix, const std::vector<double> bVector, const vnl_vector<double> resultVector, double &mean, double &stdev ) 
{
	...
	vnl_vector<double> diffVector(m); 
	for( int row = 0; row < m; row++ )
	{
		diffVector.put(row, bVector[row] - offset - aMatrix[row].get(0) * scale ); 
	}

	// calculate mean of difference 
	mean = diffVector.mean(); 

	// calculate the difference from mean 
	vnl_vector<double> diffFromMean = diffVector - mean; 

	// calculate standard deviation 
	stdev = sqrt( diffFromMean.squared_magnitude() / (1.0 * diffFromMean.size()) ); 

	// print results
	LOG_DEBUG("Mean=" << std::fixed << mean << " Std=" << stdev); 
}
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::SaveRotationEncoderCalibrationError(const std::vector<vnl_vector<double>> aMatrix, const std::vector<double> bVector, const vnl_vector<double> resultVector)
{
...
int vtkStepperCalibrationController::RotationAxisCalibration()
{
	...
	for ( unsigned int cluster = 0; cluster < clusteredFrames.size(); ++cluster )
	{
		...
		{
			double centerOfRotationPx[2] = {0, 0}; 
			this->CalculateCenterOfRotation(clusteredFrames[cluster], centerOfRotationPx); 
	...
	this->LSQRMinimizer(aMatrix, bVector, rotationAxisCalibResult); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::CalculateSpacing()
{
	...
	this->LSQRMinimizer(aMatrix, bVector, TRUSSquaredScaleFactorsInMMperPixel2x1); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::CalculateCenterOfRotation( const SegmentedFrameList frameListForCenterOfRotation, double centerOfRotationPx[2] )
{
	...
	{
		numberOfEquations = bVector.size(); 
		this->LSQRMinimizer(aMatrix, bVector, TRUSRotationCenterInOriginalImageFrameInMm2x1); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::GetCenterOfRotationCalculationError(const std::vector<vnl_vector<double>> aMatrix, const std::vector<double> bVector, const vnl_vector<double> resultVector, double &mean, double &stdev ) 
{
	...
	// calculate difference between the computed and measured distances
	vnl_vector<double> diffVector(m); 
	for( int row = 0; row < m; row++ )
	{
		diffVector.put(row, bVector[row] - aMatrix[row].get(0) * centerOfRotationX - aMatrix[row].get(1) * centerOfRotationY ); 
	}
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::RemoveOutliersFromCenterOfRotCalcData(std::vector<vnl_vector<double>> &aMatrix, std::vector<double> &bVector, vnl_vector<double> resultVector )
{
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::CalculatePhantomToProbeDistance()
{
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::RotationEncoderCalibration()
{
	...
	{
		numberOfEquations = bVector.size(); 
		this->LSQRMinimizer(aMatrix, bVector, rotationEncoderCalibrationResult); 
...
//----------------------------------------------------------------------------
void vtkStepperCalibrationController::ConstrLinEqForRotationAxisCalib( std::vector< std::pair<double, double> > listOfCenterOfRotations, std::vector< double > listOfClusterPositions, std::vector<vnl_vector<double>> &aMatrix, std::vector<double> &bVector)
{
...
void vtkStepperCalibrationController::OfflineProbeRotationAxisCalibration()
{
	...
	int frameCounter(0); 
	for( int imgNumber = this->GetSavedImageDataInfo(PROBE_ROTATION).StartingIndex; frameCounter < this->GetSavedImageDataInfo(PROBE_ROTATION).NumberOfImagesToUse; imgNumber++ )
	{
	...
	LOG_INFO ( "A total of " << frameCounter << " images have been successfully added for probe rotation axis calibration.");
...
void vtkStepperCalibrationController::OfflineProbeTranslationAxisCalibration()
{
	...
	int frameCounter(0); 
	for( int imgNumber = this->GetSavedImageDataInfo(PROBE_TRANSLATION).StartingIndex; frameCounter < this->GetSavedImageDataInfo(PROBE_TRANSLATION).NumberOfImagesToUse; imgNumber++ )
	{
		if ( imgNumber >= trackedFrameList->GetNumberOfTrackedFrames() )
		{
			break; 
	...
	LOG_INFO ( "A total of " << frameCounter << " images have been successfully added for probe translation axis calibration.");

	trackedFrameList->Clear(); 
	this->CalibrateProbeTranslationAxis(); 
}
...
void vtkStepperCalibrationController::OfflineTemplateTranslationAxisCalibration()
{
	...
	int frameCounter(0); 
	for( int imgNumber = this->GetSavedImageDataInfo(TEMPLATE_TRANSLATION).StartingIndex; frameCounter < this->GetSavedImageDataInfo(TEMPLATE_TRANSLATION).NumberOfImagesToUse; imgNumber++ )
	{
		if ( imgNumber >= trackedFrameList->GetNumberOfTrackedFrames() )
		{
			break; 
	...
	LOG_INFO ( "A total of " << frameCounter << " images have been successfully added for template translation axis calibration.");

	trackedFrameList->Clear(); 
	this->CalibrateTemplateTranslationAxis(); 
}
