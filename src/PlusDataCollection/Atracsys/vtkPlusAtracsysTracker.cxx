...
public:
  ...
  Atracsys::Tracker::DEVICE_TYPE DeviceType = Atracsys::Tracker::DEVICE_TYPE::UNKNOWN_DEVICE;
};

//----------------------------------------------------------------------------
vtkPlusAtracsysTracker::vtkPlusAtracsysTracker()
  : vtkPlusDevice()
  , InternalObj(new vtkInternal(this))
{
...
{
  std::string d;
  this->InternalObj->Tracker.GetCalibrationDate(d);
  return d;
}
...
{
  switch (this->InternalObj->DeviceType)
  {
  case Atracsys::Tracker::DEVICE_TYPE::FUSIONTRACK_250:
    return "fusionTrack250";
  case Atracsys::Tracker::DEVICE_TYPE::FUSIONTRACK_500:
    return "fusionTrack500";
  case Atracsys::Tracker::DEVICE_TYPE::SPRYTRACK_180:
    return "spryTrack180";
  case Atracsys::Tracker::DEVICE_TYPE::SPRYTRACK_300:
    return "spryTrack300";
  default:
    return "unknown";
  }
...
{
  ATR_RESULT result = this->InternalObj->Tracker.GetCamerasCalibration(
...
{
  if (this->InternalObj->Tracker.GetLoadedGeometries(geometries) != ATR_SUCCESS)
  {
...
PlusStatus vtkPlusAtracsysTracker::ReadConfiguration(vtkXMLDataElement* rootConfigElement)
{
  ...
  {
    this->InternalObj->DeviceOptions.emplace(deviceConfig->GetAttributeName(i), deviceConfig->GetAttributeValue(i));
...
{
  return this->InternalObj->DeviceOptions;
}

//----------------------------------------------------------------------------
PlusStatus vtkPlusAtracsysTracker::GetOptionValue(const std::string& optionName, std::string& optionValue)
{
  std::map<std::string, std::string>::const_iterator itd = this->InternalObj->DeviceOptions.find(optionName);
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    optionValue = this->InternalObj->DeviceOptions[optionName];
    return PLUS_SUCCESS;
  }
  return PLUS_FAIL;
}
...
bool vtkPlusAtracsysTracker::TranslateOptionName(const std::string& optionName, std::string& translatedOptionName)
{
  std::map<std::string, std::string>::const_iterator itt =
    this->InternalObj->DeviceOptionTranslator.find(optionName);
  if (itt == this->InternalObj->DeviceOptionTranslator.cend())
  {
    return false;
  }
  else
...
PlusStatus vtkPlusAtracsysTracker::InternalConnect()
{
  ...
  {
    LOG_WARNING(this->InternalObj->Tracker.ResultToString(result));
  }

  // get device serial number in hex form
  std::ostringstream oss;
  uint64 sn;
  this->InternalObj->Tracker.GetDeviceId(sn);
  oss << "0x" << std::setw(16) << std::setfill('0') << std::hex << sn;
  this->DeviceId = oss.str();

  // get device type
  this->InternalObj->Tracker.GetDeviceType(this->InternalObj->DeviceType);
  ...
  itd = this->InternalObj->DeviceOptions.find("ActiveMarkerPairingTimeSec");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    if (Atracsys::strToInt32(itd->second, this->InternalObj->ActiveMarkerPairingTimeSec))
    {
      LOG_INFO("Marker pairing time is set to " << this->InternalObj->ActiveMarkerPairingTimeSec << " seconds, tracking will not start until this period is over.");
    }
  }

  // handling options that are used both internally and in the sdk
  // ------- max registration error in mm
  itd = this->InternalObj->DeviceOptions.find("MaxMeanRegistrationErrorMm");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    if (Atracsys::strToFloat32(itd->second, this->InternalObj->MaxMeanRegistrationErrorMm))
    {
      if (this->InternalObj->MaxMeanRegistrationErrorMm < 0.1 || this->InternalObj->MaxMeanRegistrationErrorMm > 5.0)
      {
        this->InternalObj->MaxMeanRegistrationErrorMm = 2.0;
        LOG_WARNING("Invalid maximum mean registration error provided. Must be between 0.1 and 5 mm inclusive. Maximum mean registration error has been set to its default value of " << this->InternalObj->MaxMeanRegistrationErrorMm << "mm.");
      }
    }
  }
  else
  {
    this->InternalObj->DeviceOptions.emplace("MaxMeanRegistrationErrorMm",
      std::to_string(this->InternalObj->MaxMeanRegistrationErrorMm));
  }
  // ------- max number of missing fiducials
  itd = this->InternalObj->DeviceOptions.find("MaxMissingFiducials");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    if (Atracsys::strToInt32(itd->second, this->InternalObj->MaxMissingFiducials))
    {
      if (this->InternalObj->MaxMissingFiducials < 0 || this->InternalObj->MaxMissingFiducials > 3)
      {
        this->InternalObj->MaxMissingFiducials = 0;
        LOG_WARNING("Invalid maximum number of missing fiducials provided. Must be between 0 and 3 inclusive. Maximum missing fiducials has been set to its default value of " << this->InternalObj->MaxMissingFiducials << ".");
      }
    }
  }
  else
  {
    this->InternalObj->DeviceOptions.emplace("MaxMissingFiducials",
      std::to_string(this->InternalObj->MaxMissingFiducials));
  }
  ...
  itd = this->InternalObj->DeviceOptions.find("MaxAdditionalEventsNumber");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    int value = -1;
    Atracsys::strToInt32(itd->second, value);
    if (value < 0) {
      LOG_WARNING("Invalid value for max events number per frame in output: " << itd->second
        << ". Default value used (" << this->InternalObj->Tracker.GetMaxAdditionalEventsNumber() << ")");
    }
    else
    {
      this->InternalObj->Tracker.SetMaxAdditionalEventsNumber(value);
    }
  }
  // ------- maximum number of 2D fiducials (in either left or right frame) included in the device's output
  itd = this->InternalObj->DeviceOptions.find("Max2dFiducialsNumber");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    int value = -1;
    Atracsys::strToInt32(itd->second, value);
    if (value < 0)
    {
      LOG_WARNING("Invalid value for max 2D fiducials number in output: " << itd->second
        << ". Default value used (" << this->InternalObj->Tracker.GetMax2dFiducialsNumber() << ")");
    }
    else
    {
  ...
  itd = this->InternalObj->DeviceOptions.find("Max3dFiducialsNumber");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    int value = -1;
    Atracsys::strToInt32(itd->second, value);
    ...
    {
      this->InternalObj->Tracker.SetMax3dFiducialsNumber(value);
    }
  }
  // ------- maximum number of markers included in the device's output
  itd = this->InternalObj->DeviceOptions.find("MaxMarkersNumber");
  if (itd != this->InternalObj->DeviceOptions.cend())
  {
    int value = -1;
    Atracsys::strToInt32(itd->second, value);
    if (value < 0)
    {
      LOG_WARNING("Invalid value for max markers number in output: " << itd->second
        << ". Default value used (" << this->InternalObj->Tracker.GetMaxMarkersNumber() << ")");
    }
    else
    {
      this->InternalObj->Tracker.SetMaxMarkersNumber(value);
    }
  }
  ...
  std::map<std::string, std::string>::iterator itr = this->InternalObj->DeviceOptions.find("Enable_embedded_processing");
  if (itr != this->InternalObj->DeviceOptions.end())
  {
    if (this->InternalObj->DeviceOptions.at("Enable_embedded_processing") == "0")
    {
      if (this->InternalObj->Tracker.SetSpryTrackProcessingType(Atracsys::Tracker::PROCESSING_ON_PC) != ATR_SUCCESS)
      {
        LOG_WARNING("Embedded processing could not be disabled.");
        this->InternalObj->DeviceOptions.erase(itr);
      }
    }
    else if (this->InternalObj->DeviceOptions.at("Enable_embedded_processing") == "1")
    {
      if (this->InternalObj->Tracker.SetSpryTrackProcessingType(Atracsys::Tracker::PROCESSING_ONBOARD) != ATR_SUCCESS)
      {
        this->InternalObj->DeviceOptions.erase(itr);
    ...
    {
      LOG_WARNING(this->InternalObj->DeviceOptions.at("Enable_embedded_processing") << " is not a correct value for Embedded processing.\nAccepted values are 0 (false) and 1 (true).");
      this->InternalObj->DeviceOptions.erase(itr);
    }
  }

  itr = this->InternalObj->DeviceOptions.begin();
  while (itr != this->InternalObj->DeviceOptions.end())
  {
    ...
    if (itr->first == "EnableIRstrobe" && (this->InternalObj->DeviceType == Atracsys::Tracker::DEVICE_TYPE::FUSIONTRACK_500
      || this->InternalObj->DeviceType == Atracsys::Tracker::DEVICE_TYPE::FUSIONTRACK_250))
    {
      ...
      {
        if (this->InternalObj->Tracker.SetOption("Strobe mode", strobCorresp.at(itr->second)) != ATR_SUCCESS)
        {
    ...
    {
      if (this->InternalObj->Tracker.SetOption(translatedOptionName, itr->second) != ATR_SUCCESS)
      {
        itr = this->InternalObj->DeviceOptions.erase(itr);
        continue;
      }
    ...
    else if (itr->first.find('_') != std::string::npos && itr->first != "Enable_embedded_processing")
    {
      ...
      if (this->InternalObj->Tracker.SetOption(inferredOptionName, itr->second) != ATR_SUCCESS)
      {
        itr = this->InternalObj->DeviceOptions.erase(itr);
        continue;
      }
  ...
  if (this->InternalObj->DeviceType == Atracsys::Tracker::DEVICE_TYPE::SPRYTRACK_180)
  {
    if ((result = this->InternalObj->Tracker.EnableWirelessMarkerStatusStreaming(false)) != ATR_SUCCESS)
    {
      LOG_WARNING(this->InternalObj->Tracker.ResultToString(result));
    }
    if ((result = this->InternalObj->Tracker.EnableWirelessMarkerBatteryStreaming(false)) != ATR_SUCCESS)
    {
      LOG_WARNING(this->InternalObj->Tracker.ResultToString(result));
    }
  }
  ...
  for (it = begin(this->InternalObj->PlusIdMappedToGeometryFilename); it != end(this->InternalObj->PlusIdMappedToGeometryFilename); it++)
  {
    ...
    int geometryId;
    if ((result = this->InternalObj->Tracker.LoadMarkerGeometryFromFile(geomFilePath, geometryId)) != ATR_SUCCESS)
    {
  ...
  if (this->InternalObj->ActiveMarkerPairingTimeSec > 0)
  {
    // make LED blue during pairing
    this->InternalObj->Tracker.SetUserLEDState(0, 0, 255, 0);

    // pair active markers
    if ((result = this->InternalObj->Tracker.EnableWirelessMarkerPairing(true)) != ATR_SUCCESS)
    {
      LOG_ERROR(this->InternalObj->Tracker.ResultToString(result));
      return PLUS_FAIL;
    }
    LOG_INFO("Active marker pairing period started for " << this->InternalObj->ActiveMarkerPairingTimeSec << " seconds.");

    // sleep while waiting for tracker to pair active markers
    vtkIGSIOAccurateTimer::Delay(this->InternalObj->ActiveMarkerPairingTimeSec);

    LOG_INFO("Active marker pairing period ended.");

    if ((result = this->InternalObj->Tracker.EnableWirelessMarkerPairing(false)) != ATR_SUCCESS)
    {
      LOG_ERROR(this->InternalObj->Tracker.ResultToString(result));
      return PLUS_FAIL;
    }
    ...
    this->InternalObj->Tracker.SetUserLEDState(0, 255, 0, 0);

    // TODO: check number of active markers paired

    std::string markerInfo;
    this->InternalObj->Tracker.GetMarkerInfo(markerInfo);
    if (!markerInfo.empty())
    {
...
PlusStatus vtkPlusAtracsysTracker::InternalDisconnect()
{
  ...
  this->InternalObj->Tracker.EnableUserLED(false);

  ATR_RESULT result;
  if ((result = this->InternalObj->Tracker.Disconnect()) != ATR_SUCCESS)
  {
    LOG_ERROR(this->InternalObj->Tracker.ResultToString(result));
    return PLUS_FAIL;
  }
...
PlusStatus vtkPlusAtracsysTracker::PauseVirtualDevice()
{
  ...
  this->InternalObj->Tracker.Pause(true);
  return PLUS_SUCCESS;
}
...
PlusStatus vtkPlusAtracsysTracker::UnpauseVirtualDevice()
{
  ...
  this->InternalObj->Tracker.Pause(false);
  return PLUS_SUCCESS;
}
...
PlusStatus vtkPlusAtracsysTracker::InternalUpdate()
{
  ...
  ATR_RESULT result = this->InternalObj->Tracker.GetMarkersInFrame(markers, events, sdkTimestamp);

  if (result == ATR_RESULT::ERROR_NO_FRAME_AVAILABLE)
  {
    // waiting for frame
    return PLUS_SUCCESS;
  }
  ...
  for (it = this->InternalObj->FtkGeometryIdMappedToToolId.begin(); it != this->InternalObj->FtkGeometryIdMappedToToolId.end(); it++)
  {
    ...
    for (mit = markers.begin(); mit != markers.end(); mit++)
    {
      ...
      if (mit->GetFiducialRegistrationErrorMm() > this->InternalObj->MaxMeanRegistrationErrorMm)
      {
...
PlusStatus vtkPlusAtracsysTracker::AddToolGeometry(std::string toolId, std::string geomString)
{
  ...
  int geometryId;
  ATR_RESULT result;
  if ((result = this->InternalObj->Tracker.LoadMarkerGeometryFromString(geomString, geometryId)) != ATR_SUCCESS)
  {
    LOG_ERROR(this->InternalObj->Tracker.ResultToString(result) << " This error occurred when trying to load the following geometry information: " << geomString);
    return PLUS_FAIL;
  }
